...
typedef struct sms_message {
    ...
    uint32_t    status;
    char        number[32];
    char        text[SMS_MAX_LEN];
    uint64_t    timestamp;
    uint32_t    encrypted;
} sms_message_t;

typedef struct sms_thread {
    uint32_t    id;
    char        number[32];
    char        display_name[64];
    uint32_t    msg_count;
...
typedef struct phone_state {
    phone_call_t        active_call;
    phone_call_t        held_call;
    sms_thread_t        threads[SMS_MAX_THREADS];
    uint32_t            thread_count;
    phone_contact_t     contacts[1024];
    ...
    char                carrier[64];
    char                msisdn[32];
    uint32_t            voicemail_count;
    uint32_t            zrtp_enabled;
    uint32_t            call_recording_enabled;
} phone_state_t;

/* Init & status */
void phone_init(void* modem);
void phone_shutdown(void);
phone_state_t* phone_get_state(void);
...
void phone_receive_sms(const char* number, const char* text, uint64_t ts);
sms_thread_t* phone_get_threads(uint32_t* count);
sms_message_t* phone_get_messages(uint32_t thread_id, uint32_t* count);
void phone_mark_read(uint32_t thread_id);
void phone_delete_thread(uint32_t thread_id);
void phone_delete_message(uint32_t msg_id);

/* Contacts */
int phone_add_contact(const char* name, const char* number, const char* email);
void phone_remove_contact(uint32_t id);
phone_contact_t* phone_find_contact(const char* number);
...
int phone_zrtp_handshake(phone_call_t* call);
void phone_zrtp_derive_keys(const uint8_t* shared_secret, uint8_t* enc_key, uint8_t* auth_key);

/* Call recording */
void phone_set_recording_enabled(uint32_t enable);
int phone_start_recording(phone_call_t* call);
void phone_stop_recording(phone_call_t* call);
